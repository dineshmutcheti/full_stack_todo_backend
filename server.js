...
const corsOptions = {
  origin: 'https://full-stack-todo-frontend-alyb.onrender.com', // frontend URL
  methods: ['GET', 'POST', 'PUT', 'DELETE'],
};

app.use(express.json());
app.use(cors(corsOptions)); // Applying the CORS configuration
...
app.get('/read-task/:id', (req, res) => {
  ...
  db.query(q, [taskId], (err, result) => {
    if (err) {
      console.log("failed to read tasks");
      res.status(500).send("Error while retrieving the task");
    } else {
      if (result.length > 0) {
